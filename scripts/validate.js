...
for (const file of listFiles(path.join(ROOT, "agents"), ".md")) {
    ...
    for (const key of Object.keys(fm)) {
        if (!AGENT_KEYS.has(key))
            fail(id, `unknown agent frontmatter key: ${key}`);
    }
    if (fm.name !== expected)
        fail(id, `name "${fm.name}" does not match filename "${expected}"`);
    if (!fm.description) fail(id, "missing description");
    else if (fm.description.length > MAX_DESCRIPTION) {
        fail(
            id,
            `description is ${fm.description.length} chars, max ${MAX_DESCRIPTION}`,
        );
    }
    if (fm.model !== undefined && !MODELS.has(fm.model))
        fail(id, `invalid model: ${fm.model}`);
    if (fm.effort !== undefined && !EFFORTS.has(fm.effort))
        fail(id, `invalid effort: ${fm.effort}`);
    if (fm.color !== undefined && !COLORS.has(fm.color))
        fail(id, `invalid color: ${fm.color}`);
    ...
    if (!tierLine) {
        fail(
            id,
            "missing or malformed tier line (**Tier:** T<n> · **Escalates to:** <agent> · **Terminal:** yes|no)",
        );
        continue;
    }
...
for (const [name, agent] of agents) {
    if (agent.escalatesTo !== "human" && !agents.has(agent.escalatesTo)) {
        fail(
            agent.id,
            `escalates to "${agent.escalatesTo}", which is not an agent`,
        );
    }
    ...
    if (agent.terminal !== (agent.escalatesTo === "human")) {
        fail(
            agent.id,
            `Terminal: ${agent.terminal ? "yes" : "no"} contradicts "Escalates to: ${agent.escalatesTo}"`,
        );
    }
    ...
    if (agent.tier !== "T3" && agent.terminal) {
        fail(
            agent.id,
            `tier ${agent.tier} cannot be terminal; only T3 escalates to the human`,
        );
    }
...
for (const dir of skillNames) {
    ...
    for (const key of Object.keys(fm)) {
        if (!SPEC_KEYS.has(key)) {
            fail(
                id,
                `frontmatter key "${key}" is outside the Agent Skills spec; move it into metadata`,
            );
        }
    }
    if (fm.name !== dir)
        fail(id, `name "${fm.name}" does not match directory "${dir}"`);
    if (!fm.description) fail(id, "missing description");
    else if (fm.description.length > MAX_DESCRIPTION) {
        fail(
            id,
    ...
    if (doc.lines > MAX_SKILL_LINES)
        fail(id, `${doc.lines} lines, max ${MAX_SKILL_LINES}`);

    const owners = (fm.metadata && fm.metadata.owners) || [];
    for (const owner of owners) {
        if (!agents.has(owner))
            fail(id, `metadata.owners lists "${owner}", which is not an agent`);
    }
    if (fm.metadata && !fm.metadata.phase)
        fail(id, "metadata.phase is required");
...
const referenceFiles = new Set(
    listFiles(path.join(ROOT, "references"), ".md").map(
        (f) => `references/${f}`,
    ),
);
...
function checkReferences(id, body) {
    for (const match of body.matchAll(/`(references\/[a-z0-9-]+\.md)`/g)) {
        if (!referenceFiles.has(match[1]))
            fail(id, `points at ${match[1]}, which does not exist`);
    }
}
...
for (const [, agent] of agents) {
    checkReferences(
        agent.id,
        fs.readFileSync(path.join(ROOT, agent.id), "utf8"),
    );
}
...
for (const file of listFiles(commandDir, ".md")) {
    ...
    for (const match of doc.body.matchAll(/`([a-z][a-z0-9-]{3,})`/g)) {
        ...
        fail(
            id,
            `references \`${token}\`, which is neither a skill nor an agent`,
        );
    }
}
...
if (pluginManifest.agents !== undefined) {
    ...
    for (const entry of entries) {
        if (!entry.endsWith(".md")) {
            fail(
                ".claude-plugin/plugin.json",
                `agents entry "${entry}" is not a .md file path; omit the field so agents/ is scanned`,
            );
        }
    }
}
for (const entry of [].concat(pluginManifest.commands || [])) {
    if (!fs.existsSync(path.join(ROOT, entry))) {
        fail(
            ".claude-plugin/plugin.json",
            `commands path does not exist: ${entry}`,
        );
    }
}
...
for (const name of agents.keys()) {
    if (!agentsDoc.includes(`\`${name}\``))
        fail("AGENTS.md", `roster is missing ${name}`);
}
...
if (process.argv.includes("--json")) {
    ...
    console.log(
        problems.length
            ? `\nFAIL — ${problems.length} problem(s). ${counts}`
            : `PASS — ${counts}`,
    );
}
